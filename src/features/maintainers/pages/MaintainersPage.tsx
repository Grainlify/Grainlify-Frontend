...
export function MaintainersPage({ onNavigate }: MaintainersPageProps) {
  ...
  const isSetupModalOpen =
    (currentPendingProject !== null &&
      (showNewProjectModalFromRedirect || userOpenedModalForSetup)) ||
    editingProject !== null
  ...
  return (
    <div className="space-y-6">
            ...
            >
              ...
              <ChevronDown
                className={`w-4 h-4 transition-all ${isRepoDropdownOpen ? 'rotate-180' : ''} ${
                  theme === 'dark'
                    ? 'text-[#b8a898] group-hover:text-[#e8c77f]'
                    : 'text-[#7a6b5a] group-hover:text-[#c9983a]'
                }`}
              />
            </button>
                      ...
                      return (
                        <div key={group.org}>
                          ...
                          >
                            <div className="flex items-center gap-3 flex-1">
                              ...
                              {group.repos.length === 0 && (
                                <span
                                  className={`text-[12px] italic font-medium transition-colors ${
                          ...
                          {group.repos.length > 0 && isExpanded && (
                            <div
                              className={`py-1.5 space-y-0.5 transition-colors ${
                                      ...
                                      <img
                                        ...
                                        onError={() =>
                                          setFailedAvatars((prev) =>
                                            new Set(prev).add(getRepoAvatar(repo.fullName, 20))
                                          )
                                        }
                                      />
                                    )}
                  ...
                  >
                    <div className="flex items-center gap-3">
                      <div className="w-6 h-6 rounded-full bg-gradient-to-br from-[#c9983a]/30 to-[#d4af37]/20 flex items-center justify-center border border-[#c9983a]/40">
                        <Plus
                          className="w-4 h-4 text-[#c9983a] group-hover/add:text-[#b8872e] transition-colors"
                          strokeWidth={2.5}
                        />
                      </div>
                      <span
                        className={`text-[14px] font-bold group-hover/add:text-[#c9983a] transition-colors ${
                          theme === 'dark' ? 'text-[#e8dfd0]' : 'text-[#2d2820]'
                        }`}
                      >
                        Add a repository
                    ...
                    <SettingsIcon
                      className={`w-4 h-4 group-hover/add:text-[#c9983a] transition-colors ${
                        theme === 'dark' ? 'text-[#b8a898]' : 'text-[#7a6b5a]'
                      }`}
                    />
                  </button>
      ...
      {activeTab === 'Pull Requests' && (
        <PullRequestsTab selectedProjects={selectedProjects} onRefresh={refreshAll} />
      )}
