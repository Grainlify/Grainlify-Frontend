...
// LandingPage itself fetches nothing directly, but two of the sections it renders
// (Hero and WhyChooseUs) each call the shared useLandingStats() hook, which in turn
// calls getLandingStats() - confirmed by reading LandingPage.tsx, Hero.tsx,
// WhyChooseUs (inside LandingPage.tsx), and useLandingStats.ts. LandingPage also
// renders Navbar, which calls useAuth() - that requires an AuthProvider ancestor
// and AuthContext's checkAuth() reads getAuthToken() unconditionally on mount, so
// this suite renders with `withAuth: true` and the client mock below covers every
// export the mounted tree actually touches (confirmed by reading Navbar.tsx and
// AuthContext.tsx). No token is ever placed in localStorage, so checkAuth()'s
// `if (token)` branch is never entered and getCurrentUser() is never called.
