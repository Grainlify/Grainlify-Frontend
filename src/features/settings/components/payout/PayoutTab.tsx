...
export function PayoutTab() {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      const message = error instanceof Error ? error.message : 'Failed to save payout preferences'
      ...
      >
        {isLoading ? (
          ...
          <>
            ...
            <p
              className={`text-[14px] mb-8 transition-colors ${
        ...
        {isLoading ? (
          ...
          <div className="space-y-4 mb-6">
            ...
            {Array.from({ length: 4 }).map((_, idx) => (
              <div
                key={idx}
                className="grid grid-cols-2 gap-4 items-center py-4 border-b border-white/5"
              >
          ...
          <>
              ...
              {projects.map((project) => (
                ...
                >
                  <div className="flex items-center gap-3">
                    ...
                    <div
                      className={`w-10 h-10 rounded-full flex items-center justify-center ${project.owner_avatar_url ? 'hidden' : ''} ${
                        project.language
                          ? ''
                          : 'bg-gradient-to-br from-[#c9983a]/20 to-[#a67c2e]/20 border border-[#c9983a]/30'
                      }`}
                    >
                      {project.language ? (
                        ...
                        >
                          {getProjectInitial(project.github_full_name)}
                        </span>
                      )}
                    </div>
                    <div className="flex flex-col">
                      ...
                      >
                        {getProjectDisplayName(project.github_full_name)}
                      </span>
                  ...
                  <div className="flex items-center">
                    {availableProfiles.length === 0 ? (
                      <div
                        className={`px-4 py-2.5 rounded-[12px] text-[14px] transition-colors ${
                          theme === 'dark' ? 'text-[#8a7e70]' : 'text-[#7a6b5a]'
                        }`}
                      >
            ...
            {/* Info Message */}
            <div
              className={`flex items-start gap-2 p-4 rounded-[14px] backdrop-blur-[30px] border transition-colors ${
                theme === 'dark'
